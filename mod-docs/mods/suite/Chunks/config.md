...
FTB Chunks comes with 2 configuration files, one for the server and one for the client. Both files are located in the config folder of your Minecraft instance `<instance>/config/ftbchunks-(client/server).json5`

These 2 config files can be overridden on modpack updates so it also possible to locally overwrite these configs by making a copy `ftbchunks-client.json5` to `<instance>/local/ftbchunks-client.json5` and for the client and for the server copy `ftbchunks-world.json5` to `<instance>/world/serverconfig/ftbchunks-world.json5`
...
If you prefer to edit the config files manually you can do so by opening the config files in a text editor. The config files are in a JSON5 format, which is a human-readable format that is easy to edit and located in the folders mentioned above
